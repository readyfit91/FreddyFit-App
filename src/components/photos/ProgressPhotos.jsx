...
function UploadModal({ onClose, onUpload }) {
  ...
  const handleSubmit = () => {
    onUpload({
      ...
      preview,
      file,   // pass the raw File so the parent can upload to storage
    });
    onClose();
  };
...
export default function ProgressPhotos() {
  ...
  async function loadPhotos(uid) {
    ...
    setPhotos(data.map((p, i) => ({
      ...
      month:    new Date(p.photo_date + "T00:00:00").toLocaleDateString("en-US", { month: "long" }),
  ...
  const handleUpload = async (photo) => {
    ...
    if (clientId && photo.file) {
      ...
      if (!uploadErr) {
        ...
        const { data: row } = await supabase.from("progress_photos").insert({
          client_id:    clientId,
          storage_path: publicUrl,
          label:        photo.label,
          angle:        photo.angle,
          weight_lbs:   photo.weight,
          notes:        photo.notes  || null,
          photo_date:   photo.date,
        }).select().single();

        // Swap temp ID with real DB ID and URL
        if (row) {
          setPhotos(p => p.map(x =>
            x.id === photo.id ? { ...x, id: row.id, preview: publicUrl } : x
          ));
        }
        showToast("📸 Photo uploaded successfully!");
      } else {
        showToast("⚠️ Upload failed — check storage bucket permissions");
      }
    } else {
      showToast("📸 Photo saved (no Supabase storage configured)");
    }
  };

  const handleDelete = async () => {
    const toDelete = [...selected];
    setPhotos(p => p.filter(x => !toDelete.includes(x.id)));
    showToast(`${toDelete.length} photo${toDelete.length>1?"s":""} deleted`);
    setSelected([]);

    // Delete from DB
    for (const id of toDelete) {
      await supabase.from("progress_photos").delete().eq("id", id).eq("client_id", clientId);
    }
  };
