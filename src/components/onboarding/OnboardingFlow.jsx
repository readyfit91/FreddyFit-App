...
// ── STEP 4 — WELCOME ───────────────────────────────────────────────────────
function Step4({ data, onDashboard }) {
  const bmi = calcBMI(data.height, data.weight);
  const cat = bmiCategory(bmi);
  ...
  return (
    <div style={{ textAlign: "center", animation: "fadeUp 0.4s ease" }}>
      ...
      <button onClick={onDashboard} style={{ background: `linear-gradient(135deg, ${C.blue}, ${C.blueDark})`, color: C.white, border: "none", borderRadius: 12, padding: "16px 32px", fontSize: 16, fontWeight: 800, cursor: "pointer", fontFamily: ff, width: "100%", boxShadow: C.shadowLg }}>
        Go to My Dashboard →
      </button>
    </div>
  );
...
export default function OnboardingFlow() {
  ...
  const next = async () => {
    ...
    // Step 3 → 4: create the real Supabase account
    setSignUpLoading(true);
    setSignUpError("");
    const bmi = calcBMI(stats.height, stats.weight);
    ...
    if (data.user) {
      await supabase.from("profiles").upsert({
        ...
        email:           account.email,
        phone:           account.phone,
        date_of_birth:   account.dob || null,
        height:          stats.height,
        weight_lbs:      parseFloat(stats.weight) || null,
        bmi:             bmi,
        role:            "client",
        waiver_signed:   waiver.waiverSigned,
        waiver_signed_at: waiver.waiverSigned ? new Date().toISOString() : null,
        waiver_name:     waiver.waiverName,
      });
    }
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(140deg, #EFF9FF 0%, #E3F4FC 45%, #F4F7FA 100%)", fontFamily: ff, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "40px 16px 60px" }}>
      <div style={{ width: "100%", maxWidth: 540 }}>
        ...
        <div style={{ background: C.white, borderRadius: 24, padding: "36px 40px", boxShadow: "0 20px 60px rgba(26,171,227,0.12), 0 4px 20px rgba(0,0,0,0.05)", border: `1px solid ${C.border}` }}>
          ...
          {step === 3 && <Step3 data={waiver}  set={setWaiver}  next={signUpLoading ? () => {} : next} back={back} loading={signUpLoading} />}
          {step === 4 && <Step4 data={allData} onDashboard={() => navigate("/client/nutrition")} />}
        </div>
