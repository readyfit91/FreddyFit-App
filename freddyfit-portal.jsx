        ...
        <div style={{ width: 420 }}>
          ...
          <div style={{ ...S.card, padding: 36 }}>
            ...
            <button style={{ ...S.btn, width: "100%", padding: "14px 0", fontSize: 15 }}
              onClick={() => { if(form.email && form.password) onLogin(form.role, form.email); }}>
              Sign In →
            </button>
...
export default function App() {
  ...
  if (!session) return <LoginPage onLogin={(role,email) => { setSession({role,email}); setPage("dashboard"); }}/>;

  const renderPage = () => {
    if (page==="dashboard") return session.role==="trainer" ? <TrainerDashboard/> : <ClientDashboard/>;
  ...
  return (
    <div style={{ display:"flex", fontFamily:ff }}>
      <Sidebar role={session.role} page={page} setPage={setPage}
        onLogout={()=>setSession(null)} userName={name}/>
      <div style={S.main}>
        {/* Top bar */}
        <div style={S.topbar}>
